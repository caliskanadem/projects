...
team.push({ name: "Adem", surname: "Caliskan", job: "developer", age: 30 });
console.log(team);

//*team dizisindeki joblari tek tek yazdirin
team.forEach((p) => console.log(p.job));

//*age'leri bir artirarak yeni bir diziye sakla
const agesIncByOne = team.map((x) => x.age + 1);
console.log(agesIncByOne);

//* Ornek3: name ve surname'leri birlestirip buyuk harfe ceviren ve bunu fullName key'i olarak saklayan, ayni zamanda age degerlerini 5 arttirarak age key'ine saklayan ve olusan diziyi donduren kodu yazınız.
const teamFullName = team.map((p) => {
  return {
    fullName: p.name.toUpperCase() + " " + p.surname.toUpperCase(),
    age: p.age + 5,
    newJob: (p.job = "Developer"),
  };
});

console.log(teamFullName);

//* team full name de yasi 30 dan kucuk olanlarin yasini dizide sakla
...
const avgAges =
  teamFullName.reduce((sum, val) => sum + person.age, 0) / teamFullName.length;
console.log(avgAges);
